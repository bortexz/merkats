...
(defn stream-historical-trades
  ...
  [_provider instrument from to buf-or-n]
                    ...
                    (when-let [_continue? (not @stopped?_)
                      ...
                      (let [should-reverse? (t.ld/is-before next-day public-data-format-change-date)
                        ...
                        (loop [q trades-queue]
                          (when (seq q)
                            (a/alt!!
                              [[ch (vec (peek q))]] (recur (pop q))
                              stop-ch (reset! stopped?_ true))))
                        (recur (rest days))))))]
    (stream/create
     ch
     (fn []
       (a/offer! stop-ch true)
       (a/<!! process)
       (a/close! ch)))))

(defrecord HistoricalProvider []
  services/StreamHistoricalTrades
  (-stream-historical-trades [this instrument from to buf]
    (stream-historical-trades this instrument from to buf)))

(defn historical-provider [] (->HistoricalProvider))

(comment
  (def stream (stream-historical-trades
               nil
               "BTCUSD"
...
(defn- on-closed-ch
  [ch->topics connection ch]
  (let [new-ch->topics (dissoc ch->topics ch)
        prev-topics    (into #{} cat (vals ch->topics))
        new-topics     (into #{} cat (vals new-ch->topics))
        to-unsub       (set/difference prev-topics new-topics)] 
    (when (seq to-unsub)
      (try
        (bybit.websocket/unsubscribe connection to-unsub)
        (catch Exception _)))
    new-ch->topics))

(defn- on-connected-ch
  [ch->topics connection ch topics]
  (let [new-ch->topics (assoc ch->topics ch topics)
        prev-topics    (into #{} cat (vals ch->topics))
        new-topics     (into #{} cat (vals new-ch->topics))
        to-sub         (set/difference new-topics prev-topics)]
    (when (seq to-sub)
      (try
        (bybit.websocket/subscribe connection to-sub)
        (catch Exception _))) 
    new-ch->topics))

(defn- create-stream
  [{:keys [data-pub ch->topics_ connection closed?_]} topics ch]
  (if-not @closed?_
    (let [on-close (fn []
                     (doseq [t topics] (a/unsub data-pub t ch))
                     (send ch->topics_ on-closed-ch connection ch)
                     nil)]
      (doseq [t topics] (a/sub data-pub t ch))
      (send ch->topics_ on-connected-ch connection ch topics)
      (stream/create ch on-close))
    (throw (ex-info "Provider is already closed" {}))))

(defn stream-candles
  [provider instrument timeframe buf]
  (let [topics #{(candles-topic instrument timeframe)}
        ws->candle (partial ws->candle instrument timeframe)
        ch (a/chan buf (map (fn [evt] (mapv ws->candle (:data evt)))))]
    (create-stream provider topics ch)))

(defn stream-orderbook
  [provider instrument buf]
  (let [topics #{(orderbook-topic instrument)}
        ch (a/chan buf (ws->orderbook-xf))]
    (create-stream provider topics ch)))

(defn stream-trades
  [provider instrument buf]
  (let [topics #{(trades-topic instrument)}
        ch (a/chan buf (map (fn [evt] (mapv ws->trade (:data evt)))))]
    (create-stream provider topics ch)))
...
(defn stream-order-updates
  ...
  (let [xf (map (fn [{:keys [topic data]}]
        ...
        ch (a/chan buf xf)]
    (create-stream provider ch topics)))
...
(defrecord WebsocketProvider [connection
  ...
  (-stream-candles [this instrument timeframe buf] (stream-candles this instrument timeframe buf))

  services/StreamOrderbook
  (-stream-orderbook [this instrument buf] (stream-orderbook this instrument buf))

  services/StreamTrades
  (-stream-trades [this instrument buf] (stream-trades this instrument buf))

  services/StreamOrderUpdates
  (-stream-order-updates [this buf] (stream-order-updates this buf)))
...
(comment
  ...
  (def candles-stream (services/stream-candles wsr "BTCUSD" (t/new-duration 5 :minutes) (a/sliding-buffer 1)))
  (a/poll! candles-stream)
  (stream/close! candles-stream)

  (def orderbook-stream (services/stream-orderbook wsr "BTCUSD" (a/sliding-buffer 1)))
  (a/poll! orderbook-stream)
  (stream/close! orderbook-stream)
